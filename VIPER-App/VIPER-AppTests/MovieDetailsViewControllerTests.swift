...
/// The card's job is turning one projection into labels and taps into reports.
/// Which values it contains is `MovieDetailsPresenterTests`.
@MainActor
...
final class MovieDetailsViewControllerTests {
    ...
    private let presenter = MovieDetailsViewOutputSpy()
    ...
    @Test("Loading the view is reported, under the presenter's title")
    func loadingIsReported() {
        autoreleasepool {
            presenter.title = "Dune"
            let sut = makeSUT()
    ...
    @Test("Tapping the bookmark is reported")
    func bookmarkIsReported() {
        autoreleasepool {
            ...
            #expect(presenter.events.last == .didTapBookmark)
        }
    }

    @Test("Tapping Retry is reported")
    func retryIsReported() {
        autoreleasepool {
            ...
            sut.statusConfiguration?.buttonProperties.primaryAction?.performWithSender(nil, target: nil)
...
/// Records what the card reported.
@MainActor
final class MovieDetailsViewOutputSpy: MovieDetailsViewOutput {
